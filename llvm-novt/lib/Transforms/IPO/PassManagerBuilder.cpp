...
#include <llvm/Transforms/IPO/NoVTCallStatistics.h>
...
void PassManagerBuilder::addLTOOptimizationPasses(legacy::PassManagerBase &PM) {
  ...
  if (!PGOSampleUse.empty()) {
    ...
    // PM.add(new DebugLegacyPass("/tmp/debug-2.2.ll"));
  }

  // Remove unused virtual tables to improve the quality of code generated by
  // whole-program devirtualization and bitset lowering.
  PM.add(createGlobalDCEPass());
  // PM.add(new DebugLegacyPass("/tmp/debug-2.3.ll"));

  // Provide AliasAnalysis services for optimizations.
  addInitialAliasAnalysisPasses(PM);
  // PM.add(new DebugLegacyPass("/tmp/debug-2.4.ll"));
  ...
  // PM.add(new DebugLegacyPass("/tmp/debug-2.5.ll"));
  ...
  if (OptLevel > 1) {
    ...
    // PM.add(new DebugLegacyPass("/tmp/debug-2.6.ll"));
  }
  ...
  // PM.add(new DebugLegacyPass("/tmp/debug-2.7.ll"));

  // Split globals using inrange annotations on GEP indices. This can help
  // improve the quality of generated code when virtual constant propagation or
  // control flow integrity are enabled.
  PM.add(createGlobalSplitPass());
  // PM.add(new DebugLegacyPass("/tmp/debug-2.8.ll"));

  // Apply whole-program devirtualization and virtual constant propagation.
  PM.add(createWholeProgramDevirtPass(ExportSummary, nullptr));
  // PM.add(new DebugLegacyPass("/tmp/debug-2.9.ll"));
  ...
  // PM.add(new DebugLegacyPass("/tmp/debug-2.10.ll"));
  ...
  // PM.add(new DebugLegacyPass("/tmp/debug-2.10-2.ll"));
  addExtensionsToPM(EP_Peephole, PM);
  // PM.add(new DebugLegacyPass("/tmp/debug-2.10-3.ll"));
  ...
  if (RunInliner) {
    PM.add(Inliner);
    Inliner = nullptr;
    // PM.add(new DebugLegacyPass("/tmp/debug-2.10-4.ll"));
  }
  ...
  // PM.add(new DebugLegacyPass("/tmp/debug-2.10-5.ll"));

  // Optimize globals again if we ran the inliner.
  if (RunInliner)
    PM.add(createGlobalOptimizerPass());
  // PM.add(new DebugLegacyPass("/tmp/debug-2.10-6.ll"));
  ...
  // PM.add(new DebugLegacyPass("/tmp/debug-2.11.ll"));
  addInstructionCombiningPass(PM);
  // PM.add(new DebugLegacyPass("/tmp/debug-2.12.ll"));
  addExtensionsToPM(EP_Peephole, PM);
...
void PassManagerBuilder::populateLTOPassManager(legacy::PassManagerBase &PM) {
  ...
  PM.add(createGlobalDCEPass());
  PM.add(createVerifierPass());
  // PM.add(new DebugLegacyPass("/tmp/my-abi-builder-input.ll"));
  PM.add(new NoVTAbiBuilderLegacyPass());
  PM.add(new NoVTCleanupLegacyPass());
  // PM.add(new DebugLegacyPass("/tmp/my-abi-builder-output.ll"));
  PM.add(createVerifierPass());
  // PM.add(new DebugLegacyPass("/tmp/debug-1.ll"));

  addExtensionsToPM(EP_FullLinkTimeOptimizationEarly, PM);

  // PM.add(new DebugLegacyPass("/tmp/debug-2.ll"));
  ...
  // PM.add(new DebugLegacyPass("/tmp/debug-3.ll"));

  // Create a function that performs CFI checks for cross-DSO calls with targets
  // in the current module.
  PM.add(createCrossDSOCFIPass());
  // PM.add(new DebugLegacyPass("/tmp/debug-4.ll"));

  // Lower type metadata and the type.test intrinsic. This pass supports Clang's
  // control flow integrity mechanisms (-fsanitize=cfi*) and needs to run at
  // link time if CFI is enabled. The pass does nothing if CFI is disabled.
  PM.add(createLowerTypeTestsPass(ExportSummary, nullptr));
  // PM.add(new DebugLegacyPass("/tmp/debug-5.ll"));

  if (OptLevel != 0)
    addLateLTOOptimizationPasses(PM);
  // PM.add(new DebugLegacyPass("/tmp/debug-6.ll"));

  addExtensionsToPM(EP_FullLinkTimeOptimizationLast, PM);
  // PM.add(new DebugLegacyPass("/tmp/debug-7.ll"));

  if (VerifyOutput) {
    // PM.add(new DebugLegacyPass("/tmp/my-abi-builder-output-opt.ll"));
    PM.add(createVerifierPass());
  }
  // PATCH debug
  // PM.add(createPrintModulePass(llvm::errs(), "===== AFTER ALL PASSES ====="));
  //if (OptLevel <= 1)
  //  PM.add(createCallStatisticsPass());
}
